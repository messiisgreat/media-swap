...
const informationTransfers = [
  ...
  },
  {
    ...
    content:
      "Google Analytics には、各ページの訪問回数や滞在期間、利用条件や地域別のユーザー数、アクセス源や検索フレーズ等の解析ツールが含まれています。また、Google Analyticsの識別子を、他のサイトをまたいで個別のユーザーやブラウザを追尾する目的で利用することは許されていません。",
  },
  {
    title: "機能補助",
    content:
  ...
  },
  {
    ...
    content:
      "Google Tag Manager はウェブサイトやアプリケーションで使われる第三者提供のサービスのコードを統制し、ユーザーのアクションや表示ページ、利用環境に応じて、適切な第三者のサービスを必要に応じて起動する機能を持っています。",
  },
...
const cookies = [
  {
    title: "(1) Cookie とは",
    content:
      "本サービスのウェブページは、弊社グループまたは弊社グループが認可した第三者によってCookieを活用し、お客様の端末情報を取得しております。ここでいう「Cookie」とは、お客様がウェブサイトを閲覧するとき、その使用デバイスに保存されるとともに、ウェブサーバとブラウザ間で交換される、特定の情報を持つ小さなテキストデータのことを指します。\nこのウェブサーバは、訪れたデバイスの Cookie からの情報を読み取る能力があります。たとえば、ウェブサーバは、Cookie 内に保存された識別情報（Cookie ID）をチェックし、お客様のブラウジングツールを特定できます。さらに、お客様が以前に入力した内容や選んだ設定を Cookie に保持することにより、次回からの入力や設定の手間を減らし、より便利にサービスを享受できるようにすることも可能です。",
  },
  {
    title: "(2) Cookie の利用停止",
    content:
      "お客様は、ブラウザの設定を通じて Cookie の活用を制限し、さらに、お客様のデバイス上にある Cookie を除去することができます。しかしながら、Cookie の活用を制限すると、本サービスの正常な動作が妨げられる、あるいは Cookie が必要な広告の設定が適用されないリスクが生じます。",
  },
] as const satisfies readonly Item[];

const externalTransmissions = [
  {
    title: "1. お客様端末からの第三者への情報転送に関する説明",
    content: (
      <>
        <p>
        ...
        {informationTransfers.map((informationTransfer) => (
          <div key={informationTransfer.title}>
            <H className="text-lg font-bold">{informationTransfer.title}</H>
            <p>{informationTransfer.content}</p>
          </div>
        ))}
      </>
    ),
  ...
  {
    title: "2. お客様のデバイスから私たちが受け取る情報の詳細",
    content: (
      <>
        <div className="py-2">
  ...
  {
    title: "3. Cookie の利用について",
    content: cookies.map((cookie) => (
      <div key={cookie.title} className="py-2">
        <H className="text-lg font-bold">{cookie.title}</H>
        <p>{cookie.content}</p>
      </div>
    )),
  },
    ...
    <div className="border-2">
      ...
      <Section>
        <p className="py-6">
          ...
          {CORPORATE_NAME}のうち、
          <TextLink href="/privacy-policy">プライバシーポリシー</TextLink>
          を適用する事業体
